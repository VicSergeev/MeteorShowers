...
struct SunRiseSetCalc {
    func calculateSunriseSunset(for date: Date = Date(), at location: CLLocation) -> (sunrise: Date?, sunset: Date?) {
        ...
        let solarNoon = (720 - 4.0 * longitude - eqTime) / 1440.0
        ...
        // Calculate sunrise and sunset times (in hours from midnight UTC)
        let sunriseHour = solarNoon - haHours / 24.0
        let sunsetHour = solarNoon + haHours / 24.0
        
        // Convert hours to date components
        var riseComponents = calendar.dateComponents([.year, .month, .day], from: startOfDay)
        riseComponents.hour = Int(sunriseHour * 24.0)
        riseComponents.minute = Int((sunriseHour * 24.0).truncatingRemainder(dividingBy: 1.0) * 60.0)
        
        var setComponents = calendar.dateComponents([.year, .month, .day], from: startOfDay)
        setComponents.hour = Int(sunsetHour * 24.0)
        setComponents.minute = Int((sunsetHour * 24.0).truncatingRemainder(dividingBy: 1.0) * 60.0)
        ...
        // Convert to local time (only once)
        ...
        🌅 Debug Sun Calculation:
           ...
           Solar Noon (UTC): \(solarNoon * 24.0) hours
           Sunrise Hour (UTC): \(sunriseHour * 24.0) hours
           Sunset Hour (UTC): \(sunsetHour * 24.0) hours
